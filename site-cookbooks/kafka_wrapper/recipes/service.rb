...
template "/etc/default/kafka" do
  ...
  mode "0644"
  variables(
    :kafka_home => ::File.join(node["kafka_wrapper"]["install_dir"], version_tag),
    :kafka_config => config_dir,
    :kafka_bin => bin_dir,
    :kafka_user => node["kafka_wrapper"]["user"],
    :scala_version => node["kafka_wrapper"]["scala_version"],
    :kafka_heap_opts => node["kafka_wrapper"]["kafka_heap_opts"],
    :jmx_port => node["kafka_wrapper"]["jmx"]["port"],
    :jmx_opts => node["kafka_wrapper"]["jmx"]["opts"]
  )
  notifies :restart, "service[kafka]", :delayed
end

case node["kafka_wrapper"]["service_style"]
when "upstart"
